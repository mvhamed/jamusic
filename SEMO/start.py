...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "Zo_Mbi_e":
      try:
        ...
        pass
      return await message.reply_text(f"**♪ انضم المطور جاكو للشات  💎 .\n♪ مرحبا بك : @Mvhmed  💎 .**")
    dev = await get_dev(bot_username)
...
"مره واحد اسمو زومبي  كان بيرجم ابليس ف الحج قالولو ليه؟ قال عشان يمكن احتاجو 👻😹",
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝐬𝐨𝐮𝐫𝐜𝐞 𝙹𝙰𝙺𝙾𝙾](t.me/Mvhmed) 💎 .
么  [𝒅𝒆𝒗 𝙹𝙰𝙺𝙾𝙾](t.me/Mvhmed) 💎 .
╰──── • ◈ • ────╯
...
@Client.on_message(filters.command(["المطور جاكو","جاكو","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="Zo_Mbi_e")
...
@Client.on_message(filters.command(["الغشيم","المطور چاكو ","المطور غشيم"], ""))
...
async def set_vi_so(client: Client, message):
   NAME = await client.ask(message.chat.id,"**♪ ارسل رابط لوجو السورس  💎 .\n♪ مثال ⟨ https://telegra.ph/file/1860b3cdf99b31e50dab5.jpg ⟩  💎 .**", filters=filters.text, timeout=30)
   VID_SO = NAME.text
   bot_username = client.me.username
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["Zo_Mbi_e"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["Zo_Mbi_e"]:
   ...
   if message.from_user.username == "Zo_Mbi_e":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور جاكو  💎 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @Mvhmed  💎 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @mvhmed  💎 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @mvhmed  💎 .**")
...
async def qurann2(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @Mvhmd  💎 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["Zo_Mbi_e"]:
         await message.reply_text("**♪ رتبتك هي : المطور زومبي  💎 .**")
         return
